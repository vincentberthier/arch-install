...
doas openconnect --background --protocol=gp YOUR_VPN_SERVER --user=YOUR_USERNAME
...
doas pkill -f openconnect

# Restore DNS
if [[ -f /etc/resolv.conf.pre-vpn ]]; then
    doas cp /etc/resolv.conf.pre-vpn /etc/resolv.conf
fi
...
setup_display_manager() {
    ...
    if lspci | grep -i nvidia &>/dev/null; then
        ...
        sddm_gpu="GreeterEnvironment=QT_QPA_PLATFORM=wayland,GBM_BACKEND=nvidia-drm,__GLX_VENDOR_LIBRARY_NAME=nvidia"
    fi
...
main() {
    ...
    echo "2. Configure 1Password integration"
    echo "3. Set up 'chezmoi init https://github.com/vincentberthier/dotfiles.git'"
    echo "4. Apply chezmoi conf 'chezmoi apply' twice"
